...
echo "3. Install the mitmproxy CA (only needed once - it is stable across rebuilds):"
echo "     Firefox → Settings → Certificates → View Certificates → Authorities → Import"
echo "     → $REPO_ROOT/certs/mitmproxy-ca-cert.pem  (or browse to http://mitm.it via the proxy)"
